                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <form action="{{ route('update.slider') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="old_image" value="{{ $edit_slider->slider}}">
                                <input type="hidden" name="id" value="{{ $edit_slider->id}}">
                                <div class="mb-3">
                                    <label for="slider" class="form-label">Slider:</label>
                                    <div class="controls">
                                        <input type="file"  name="slider" class="form-control" value="{{asset($edit_slider->slider)}}"
                                            onchange="mainthumbUrl(this)">
                                        @error('slider')
                                            <span class="text-danger"> {{ $message }} </span>
                                        @enderror
                                    </div>
                                    <img src="{{ asset($edit_slider->slider) }}" id="mainThumb"
                                            style="width: 40%; height:40%; padding-top:10px;">
                                </div>

                                <div class="mb-3">
                                    <label for="caption" class="form-label">Caption:</label>
                                    <input type="text" name="caption" class="form-control" value="{{$edit_slider->caption}}" id="caption">
                                    @error('caption')
                                        <span class="text-danger"> {{ $message }} </span>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="title" class="form-control" value="{{$edit_slider->title}}" id="title">
                                    @error('title')
                                        <span class="text-danger"> {{ $message }} </span>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="description" class="form-control" id="description" value="{{$edit_slider->description}}">
                                    @error('description')
                                        <span class="text-danger"> {{ $message }} </span>
                                    @enderror
                                </div>
